...
- **Xcode project**: `app/project.yml` (XcodeGen) is the source of truth. After editing it, run `cd app && xcodegen generate` (or `make regen`) to regenerate `LoomClone.xcodeproj`. The generated project is gitignored with one exception — `project.xcworkspace/xcshareddata/swiftpm/Package.resolved` is tracked, because it's the only record of which transitive dependency versions the app was built against. `xcodegen generate` leaves it alone; a `rm -rf LoomClone.xcodeproj` will show it as deleted until the next build restores it, so `git restore` it rather than committing the deletion. **Also run `make regen` after adding or removing any `.swift` file** — sources are discovered at generate time, not at build time, so xcodebuild won't find them otherwise.
