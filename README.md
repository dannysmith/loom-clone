...
Loom works, but I don't own my URLs, can't switch between camera and screen mid-recording, the interface is cluttered with features I don't use, and Atlassian keeps adding AI things I didn't ask for. Cap is open-source and lets me use my own domain, but it feels half-baked — things break randomly and it's not something I'd bet a permanent video library on.
...
2. **Server** — Hono + Bun on a Hetzner VPS. Receives streamed segments during recording, runs post-processing, stores everything.
...
4. **Viewer Surface** — Public-facing video pages, embeds, feeds and metadata endpoints. Served through BunnyCDN.
...
- **Transcription** — runs on-device via WhisperKit on the Mac. The transcript powers subtitles in the player, full-text search in the admin panel, and machine-readable metadata on the public site. A suggested title is also generated from the transcript using Apple's on-device foundation models.
...
- Vidstack-based player with poster image, subtitles from the generated transcript, and storyboard hover previews when scrubbing longer videos.
...
- oEmbed endpoint for automatic rich previews in Notion, Slack, WordPress and anything else that supports oEmbed discovery.
- RSS feed (`/feed.xml`) and JSON Feed (`/feed.json`) for public videos, with media enclosures, thumbnails and truncated transcripts.
- `llms.txt` dynamically generated with endpoint documentation and a list of public videos — built for AI agents and programmatic consumers.
