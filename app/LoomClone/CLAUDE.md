...
- **RecordingActor** — orchestrates everything. Owns the metronome (emit loop at the target rate, 30 or 60fps), the camera frame queue, the recording clock anchor, pause/resume state, and all PTS retiming. Entry point for all capture callbacks. Split across ten extensions: `+Prepare` (two-phase start), `+Metronome` (drift-corrected emit loop + the 2 Hz health task), `+FrameHandling` (capture callbacks, PTS retiming, metronome frame emission), `+KeepAlive` (held frames for a source that isn't changing), `+Stop` (stop/teardown ordering), `+SourceHealth` / `+QualityHealth` (staleness and quality warnings), `+Exclusion` (ScreenCaptureKit content filter, focused-window warning), `+CompositionRecovery` (GPU failure handling and terminal escalation), `+Diagnostics` (diagnostics.json and frame-level counters).

  Four concerns live in pure value types the actor owns, so they're testable without a running pipeline: `RecordingClock` (all PTS arithmetic, the freshness gate, keep-alive and pause accounting, the commit anchor, metronome drift correction), `SourceHealthTracker` (per-source staleness/failure bookkeeping), `AppExclusionState` (filter-refresh throttle, focused-window warning state machine) and `CameraCadenceMonitor` (delivered-rate measurement and the quality-degradation predicate `+QualityHealth` reads). Change the maths there, not inline — that's where the tests are.
...
- **screenAndCamera**: the latest screen frame (single-slot cache) is composited with the most recent camera frame peeked (without popping) as the PiP overlay. Cadence follows whichever of the two captured most recently, so a static display still records at camera rate rather than holding at the keep-alive's 1fps.
...
`CameraCaptureManager.bestFormat(for:maxHeight:targetFPS:)` picks the highest-resolution format under the preset's height cap whose advertised **max** rate reaches `targetFPS.minAcceptableRate` (a 1 fps tolerance, so NTSC fractional rates like 29.97 / 59.94 pass — only the top of the range is checked, not containment). `capFrameRateIfSupported` then sets at most `activeVideoMinFrameDuration` — a **ceiling** on frame rate ("don't deliver faster than target") — and **never** `activeVideoMaxFrameDuration` (a *floor*). For discrete `min == max` ranges it uses the range's own reported duration, not `CMTime(1, fps)`, to dodge an uncatchable `NSInvalidArgumentException`.
