#!/usr/bin/env bash
#
# Tier 3: three-writer combinations, including the known-hang config
# (T3.2). Unlike Tier 1/2 this runner takes ONE config name per
# invocation and refuses to batch — the hard constraint from the task
# doc is that Tier 3+ configs run alone so that a wedge lands on a
# single last-known-good marker, not inside a sweep.
#
# Safety contract:
...
# Usage:
#   ./run-tier-3.sh T3.1
#   ./run-tier-3.sh T3.1-phase-2-1080p-stable-baseline
#   ./run-tier-3.sh --dry-run T3.2
#   ./run-tier-3.sh --list
#
# The first form prefix-matches the configs in test-configs/tier-3/.

set -u

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
TIER_DIR="$SCRIPT_DIR/test-configs/tier-3"
...
if [ -z "$TARGET" ]; then
    cat <<EOM >&2
error: Tier 3 runs one config at a time — pass the config name.
...
usage:
  $(basename "$0") T3.1
  $(basename "$0") T3.1-phase-2-1080p-stable-baseline
  $(basename "$0") --dry-run T3.2
EOM
    exit 2
fi
...
if [ ${#matches[@]} -eq 0 ]; then
    echo "error: no Tier 3 config matches '$TARGET'" >&2
    echo "available:" >&2
    for config in "$TIER_DIR"/*.json; do
        echo "  $(basename "$config" .json)" >&2
    done
