...
## Outcomes (2026-09-05, #66 phase 3)

Section-by-section closeout, recorded when the review series (#57–#66) completed:

- **§1 Docs drift** — Addressed. #58 fixed every instance (re-verified 14/14 in the triage above). The procedural half stuck: every subsequent task updated its affected docs in-line, and the periodic docs-audit has now run twice (the triage, and the #66 full sweep).
- **§2 CI gap** — Addressed by #57, beyond the recommendation: ffmpeg installed *and* version-guarded in CI, tests run inside the digest-pinned production base image, and CI fails on any unexpected skipped test so the media suite can't silently stop running again.
- **§3 Silent ops failure** — Addressed by #61: healthchecks.io dead-man's switch on backup/restic-check/daily self-check, Docker `HEALTHCHECK`, daemon-level log rotation (verified + documented), the `healing` timeout sweep + filter + idempotency guard, and the compose env trap removed. Verified end to end with a live alert-email test. Consciously declined within it: automatic processing retry (alert instead) and 404/error-spike monitoring.
- **§4 Backup edges** — Addressed by #61: `chapters.json` backed up, restore loop at full parity with the backup list, alerting promoted to required, and a retention bug the review didn't spot (restic's host+paths grouping never pruning superseded groups) found and fixed along the way. Consciously declined: `captions.srt` (re-derivable from `words.json`); the restore drill stays deferred; R2 stays icebox. The static-export note this section asked for exists now: `docs/developer/static-export-end-state.md`.
- **§5 store.ts re-layering** — Addressed by #62: `DATA_DIR` lives in a dependency-free `paths.ts`, orchestration in `lifecycle.ts`, the dynamic-import cycle-breakers are gone, and the directional rule ("new orchestration goes beside the store") is codified in `server/CLAUDE.md`.
- **§6 Pipeline orchestrator** — Went further than the advised containment: #63 dissolved the mode matrix rather than freezing it — edit stopped being a run mode, the presentation-master split removed the in-place rewrites, and the `ctx.duration` side channel became an explicit named field.
- **§7 macOS app** — Addressed by #65 (extractions, pure-function test seams, the de-duplication inventory — re-verified in the triage: shared stop flow, capture-session observers, `LocalRecordings`, `putJSON`, typed enums, dead code gone). App CI remains consciously declined — a project with real costs, decided separately.
- **§8 Frontend gates** — Addressed by #60 + #64: admin-client under Biome/typecheck/CI, editor built in CI, the committed player bundle verified against source, multi-stage Dockerfile, fail-loud env-flag dev detection, packages merged. Cover tool documented and de-hardcoded. Admin-surface CDN scripts confirmed deliberate policy (post-review note above).
- **§9 AI-authorship residue** — The inventory is fully dispositioned (#59 + the triage above: fixed or rejected with recorded reasons, including the emblematic `purgeVideo` bare-slug purge, now pinned by tests). The sibling-grep habit is cultural rather than enforced; the addenda in this file are the record that it's being practised.
- **§10 Principles** — No action was required; worth noting that never-lose-footage gained the most (CI + ops hardening), and the URL-permanence long-horizon item is now written down as a constraint.
- **"If you only do five things"** — 1 ✅ #57 · 2 ✅ #61 · 3 ✅ #58 + habit · 4 ✅ #60 · 5 ✅ #61 (backup edges; the rehearsed restore remains the one consciously deferred piece).
