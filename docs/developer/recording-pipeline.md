...
- **Anchor:** At commit time (T=0), `recordingStartTime` is set to the most recent cached source frame's hardware capture PTS — but only when that frame is fresh. `RecordingClock.commitAnchor` clamps the anchor to `now − maxCommitAnchorAge` (100 ms) when the cached PTS is absent, in the future, or older than that, logging "clamping anchor". Not `CMClockGetTime`, not `Date()`.
...
Two bootstrap details make this reachable from the very first tick, which is what stops a recording started on a static display from having no picture at all until the display changes:
...
When a capture source dies or stalls mid-recording, the failure is detected, surfaced to the user as a warning pill in the recording panel, and recorded in the timeline. Detection lives in `RecordingActor+SourceHealth.swift` (thresholds and the fire-once/clear/re-fire bookkeeping are in `Helpers/SourceHealthTracker.swift`, pure + unit-tested); the UI is `WarningBannerView` / `WarningPill` driven by `RecordingCoordinator.activeWarnings`. Three complementary mechanisms run:
...
- **Wiring.** Fed from `recordCameraFrameForDiagnostics` (`RecordingActor+Diagnostics.swift`, the existing camera measurement point) and evaluated from the health loop. On transition it fires a single `.qualityDegraded` pill (`.warning` severity — output is still being produced; the user's call to pause/stop/carry on) and records `quality.degraded` / `quality.recovered` timeline events carrying the windowed non-monotonic count and measured camera fps. A forensic `cameraNonMonotonicPTS` counter is also carried in `diagnostics.json` counters + periodic snapshots so a recording's firing window can be cross-checked against the extracted `-12743` flood.
- **Preview.** The same `CameraCadenceMonitor` runs in `CameraPreviewManager` (the preview shares the device + CMIO path, so a stuttering preview predicts a stuttering recording). When the preview feed goes non-monotonic it sets `previewFeedUnstable`. This, plus a structural rate check, surfaces in the popover preview pane as the `CameraHealthNote` leaf (see [Coordinator and UI](#coordinator-and-ui)). There is no clean app-level API to reset a wedged USB/CMIO device, so the guidance is reconnect / different port / HDMI, not a magic reset.
...
`CameraCaptureManager` forwards session runtime errors and interruptions to `handleCameraSessionError` / `handleCameraSessionInterrupted` (→ `source.camera.failed` + pill); a silent stall trips `source.camera.stale` after 1s.

An **interruption is recoverable and a stream error isn't**, and the two are handled differently. `AVCaptureSession.interruptionEndedNotification` takes the failure warning back down (`SourceHealthTracker.markFailureEnded`), which matters beyond the pill: `cameraIsUnusable` gates the PiP composite *and* whether the camera drives output timing in `screenAndCamera`, so a permanent failure costs the recording its camera for good. Recovery is notification-driven, never inferred from a frame arriving — a source can deliver a stray buffer while still broken. Clearing the failure isn't a promise that frames resumed either: the stall clock re-arms from the recovery moment, so if they don't, the watchdog says so a threshold later with the honest message. An `SCStream` `didStopWithError` has no such notification and its warning stays sticky, which is correct — that stream really is dead.

At the pipeline level the FIFO drains naturally — in `cameraOnly` the metronome skips every tick; in `screenAndCamera` the PiP overlay disappears but screen recording continues.
...
- **Inputs:** which display, camera, mic were used (ids, names, dimensions). For cameras v3 additionally carries an `advertisedFormats` list (deduplicated by `(width, height, maxFrameRate)`) and a `selectedFormat` block — what AVCaptureSession actually picked plus `didLockRate`, which (schema name kept for back-compat) now records whether a rate *ceiling* was applied (`didCapRate`), not whether the target sat inside the format's range — a rate-locked format like the ZV-1's `30–30` reports `false`.
- **Preset:** output dimensions and bitrate.
- **Segments:** per-segment index, filename, bytes, duration, upload status.
- **Events:** timestamped log of commits, stops, mode switches, pauses, resumes, segment emissions, upload outcomes, composition failures/rebuilds, and source-health events (`source.*.stale`/`.failed`/`.recovered`, `audio.failover`, `writer.hls.failed`, and `raw.writer.failed` carrying its top-level **and** deepest-underlying NSError `code`/`domain`). v3 adds `keepalive.emitted` (one per static run) and `monotonicity.rejected` (should be zero on healthy recordings; capped at 32 events per recording, after which a one-shot `monotonicity.rejected.suppressed` sentinel is emitted and further rejections only update counters/histograms). The `chapter.marker` kind carries `data: { id: "<uuid>" }` and is emitted when the user presses the bookmark button in the recording panel — `t` is the user-visible clock time (frozen at the pause-start value if the recording was paused at the moment of the press).
...
Schema and ring-buffer semantics are documented on `MetronomeDiagnostics.FullDump` in `Pipeline/MetronomeDiagnostics.swift`.
...
- **State machine:** `.idle` → `.countingDown` → `.recording` (⇄ `.paused`) → `.stopped` → `.idle`. SwiftUI observes this for panel content.
- **Device enumeration:** Refreshes display/camera/mic lists when the popover opens; hot-plug refreshes are notification-driven (the 2s poll loop only checks server health).
- **Preview lifecycle:** Starts/stops camera and screen previews based on popover visibility, source selection, and current mode. Previews are torn down when not visible to save resources. The camera preview also publishes live `previewMetadata` (delivered resolution + measured/advertised frame rate range); the popover shows a subtle `CameraMetadataBadge` of the facts, plus the `CameraHealthNote` which interprets them into a pre-record warning. The note prioritises the **#30 desync shape** — a *rate-locked* format (only one advertised rate, via `CameraCaptureManager.shouldCapRate`) that the camera is measurably *not sustaining* ("Camera can't sustain 30fps — may desync"). That combination is precise: a multi-rate camera delivering below target (e.g. a Cam Link at 26 against 30, floor 25) is fine and stays silent, while a 30-only camera delivering 25 is flagged. It falls back to the generic `previewFeedUnstable` instability note when the rate shape isn't the issue. Both are leaf subviews so the ~1Hz metadata reads don't invalidate the `NativePopUpPicker`-hosting parent (picker-flood note).
