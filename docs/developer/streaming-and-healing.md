...
| `incomplete` | given up on (never `/complete`d, or a heal that stalled); serves whatever partial HLS exists | partial HLS |
...
The admin video page renders a **readiness checklist** (✅/❌/⏳/—) and a derived badge (`ready · enriching (N left)` / `awaiting transcript` / `complete ✓`) from `computeReadiness()`. Reprocess controls are dependency-aware via `reprocessability()` (`canRebuildSource` / `sourceValid` / `dataLoss`): a global **"Re-run post-processing"** (`POST /reprocess`, a `present` run that rebuilds the presentation set and honours whatever EDL is on disk), **"Rebuild from HLS"** (`POST /reprocess` with `rebuild=hls`, an `intake` run — only when the HLS/upload source still exists), and per-row **"↻" regenerate** buttons (`POST /reprocess/:kind`) for the steps in `REGENERABLE_KINDS` — everything except `source` and `presentation`, the two steps with dependents that would be left describing a file that no longer exists. When nothing can be rebuilt, a data-loss message replaces the buttons. A daily sweep (`markStalledVideosIncomplete`) marks stalled videos `incomplete` — `recording` with no segment activity for >4h, `healing` with none for >48h (generous, because a heal legitimately waits for a Mac that may be offline for days; the move is reversible — a re-`/complete` from `incomplete` triggers an intake reprocess). The dashboard **"Needs attention"** filter (`?attention=1`) surfaces `processing_failed` / `incomplete` / `healing` / stalled-`processing` videos.
