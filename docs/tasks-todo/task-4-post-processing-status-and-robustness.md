...
> **Phase 3:** per-artifact regenerate (pipeline `only`/`force`, `REGENERABLE_KINDS`, `POST /reprocess/:kind`); `reprocessability()` (canRebuildSource / sourceValid / dataLoss); the two-pipeline distinction (resumable vs forced `rebuild=hls`) with data-loss handling; and the checklist regenerate buttons + Mac-sent suggestion grouping.
...
> **Remaining:** Phase 4 (final simplification pass). **Known not-yet-done:** the edit-pipeline still regenerates its multi-file output set with per-file tmp→rename rather than the **atomic-set staging** the doc calls for (stage the full regenerated set to a side location, validate all of it, then swap) — an interrupted edit can briefly leave a new `{H}p.mp4` beside a stale variant/VTT. Tracked here for a conscious decision in Phase 4 or a follow-up.
